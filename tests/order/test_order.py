...
async def test_order_processing(mocker):
    mocker.patch('ecommerce.orders.mail.send_email_background', return_value=False)
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        order_response = await ac.post("/orders/add", headers={'Authorization': f'Bearer {user_access_token}'})
...
async def test_order_listing():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        user_access_token = create_access_token({"sub": "test@gmail.com"})
        response = await ac.get("/orders/all", headers={'Authorization': f'Bearer {user_access_token}'})
