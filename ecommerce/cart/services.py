...
async def add_to_cart(current_user: user_schema.User, product_id: int, db_session: Session) -> dict:
...
async def get_all_items(current_user: user_schema.User, db_session: Session) -> schema.ShowCart:
    user_info = db_session.query(user_models.User).filter(user_models.User.email == current_user.email).first()
    return db_session.query(models.Cart).filter(models.Cart.user_id == user_info.id).first()


async def remove_cart_item(current_user: user_schema.User, cart_item_id: int, db_session: Session) -> None:
    user_info = db_session.query(user_models.User).filter(user_models.User.email == current_user.email).first()
